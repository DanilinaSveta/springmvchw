...
public class UserRestController {
 ...
 /*   @GetMapping("/users")
    ...
    @PutMapping("/user/{id}/{name}/{email}")
    public void updateUsers(@PathVariable int id, @PathVariable String name, @PathVariable String email) {
        userCRUDService.update(id-1, new User(id, name, email));
        System.out.println("Обнавлен пользователь "+
                userCRUDService.getUserDb().get(userCRUDService.getUserDb().size() - 1));
    };
}